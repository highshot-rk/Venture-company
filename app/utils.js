...
export function getSelectedNavigation(navigation, navDepth) {
  let result = navigation;
  navDepth.map((item, index) => {
    if (index === 0) {
      result = result.filter(n => n.name === item)[0]
    } else {
      result = result.children.filter(r => r.name === item)[0]
    }
  })
